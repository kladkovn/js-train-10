...
function customObjectFromEntries(entries) {
  ...
  const customObject = entries.map((key) => console.log(key)); // Використання методу `map` для обробки значень властивостей
  
    // Перевірка, чи ключ  є числом
    // Перетворення числового значення на рядок
    // Повернення обробленого запису [key, value]
  const result = Object.fromEntries(entries); // Використання методу `fromEntries` для створення об'єкта з масиву записів
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const result = Object.assign(obj1, obj2);   // Використовуємо метод Object.assign() для додавання властивостей obj2 до obj1
  Object.freeze(result);    // Використовуємо метод Object.freeze() для "заморожування" об'єкта, запобігаючи подальшим змінам
  console.log(Object.isFrozen(result));    // Перевіряємо, чи об'єкт "заморожено"
  return result;    // Повертаємо "заморожений" об'єкт, якщо все пройшло успішно
...
function keysAndValues(obj) {
  ...
  return result; // Повертаємо масив з масивами ключів та значень
}
...
function filterObjectsByKey(arr, key) {
  ...
  const result = arr.filter((key) => key);  // Використовуємо метод filter() для вибірки об'єктів, що містять вказаний ключ
...
function checkAndPreventExtensions(obj) {
  ...
  Object.preventExtensions(obj);    // Перевіряємо, чи можна додати нові властивості до об'єкта
   // Якщо можна, запобігаємо подальшому додаванню властивостей
  console.log(obj);    // Повторно перевіряємо, чи можна додати нові властивості до об'єкта, та повертаємо результат
}
...
function sealAndCheck(obj) {
  ...
  console.log(Object.isSealed(obj));  // Перевіряємо, чи запечатаний об'єкт, та повертаємо результат
}
...
function getObjectValuesSum(obj) {
  ...
  const arr = (console.log(Object.values(obj)));
...
function convertArrayToObj(arr) {
  ...
  for(let i = 0; i <= arr.length; i++);    // Проходимося по кожному підмасиву в масиві за допопмогою циклу for, лічильник від нуля до довжини масиву
    ...
    Object.fromEntries(arr);  // Додаємо ключ та значення до об'єкта
